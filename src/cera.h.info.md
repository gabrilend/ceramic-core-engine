...
Every call a program built with this engine can make, and nothing else.
With `cera.c` beside it this is the engine: two files, one include path,
and nothing to install.

The header itself is declarations and fold markers with almost no prose,
because the prose is here. Both files carry the same eleven components
in the same order, and within a component the header declares calls in
the order the body defines them — `tests/117-test-header-mirrors-body.sh`
