...
- A construction failure routine that prints what was wrong with the
  ...
  helps nobody.
- Ring buffers start at ten slots on purpose: growth is cheap and
...
- Every port gets its slots at placement whatever it is for, and
...
- Ports are appended at the tail of their list so indices match the
  ...
  swaps the pointer. `out_port_dests` reads it with one atomic load and no
...
- Teardown walks stations, then input ports, then output ports,
  freeing leaf-first, and then empties the scrapyard.
