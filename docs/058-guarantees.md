...
| T5 | A task comes into existence by exactly one path. | Everything that starts a station — a delivery arriving, a static being written, the statics bound while a program is built, a captured queue being put back — reaches the same readiness check and the same task build. One door, not several. | structure |
