...
| V2 | A station pairs whatever is at the head of each of its ports. | There is no notion of a round, a batch, or a matching set, and nothing could implement one without a tag riding on every value. | structure |
...
| # | always true | what it costs | held by |
|---|---|---|---|
| S1 | A station never moves once placed. | Today the table is one flat allocation fixed at creation, so the number of stations cannot grow. Issue 211 lets it grow by adding a **shelf** — another allocation, with a short list of where they are — which keeps this guarantee rather than trading it away, because nothing already placed is copied. It has to be kept: a station holds its own mutex, and a mutex is identified by where it lives, so moving one would leave every thread parked on it waiting at an address nobody unlocks. | structure |
| S2 | A wire never names a station that is not there. | A wire is an index into the map's station array, never a pointer. Stations were once never removed at all, which held this trivially; issue 216 allows removal and holds it properly instead &mdash; **removing a station is what removes the wires to it**, walking every station's output ports first, so no stale wire can survive to be followed. That walk is the price, and it is paid during a rare operation rather than on every delivery, which is what a version tag on every wire would have cost. | structure |
| S3 | Growing an input buffer invalidates nothing. | Only the storage the port points at is reallocated — never the port, never the station — so every wire and every in-flight value survives. | structure |
| S4 | An input buffer is never full when a value arrives. | It grows before a writer can fail to find an empty cell. Issue 210c replaced the head-and-tail indices with a per-cell state, so "full" stopped being a computed condition and became a question asked directly &mdash; nothing answers a search for an empty cell &mdash; and issue 210e makes the growth an appended page rather than a doubling copy, because a claimer copying bytes outside the lock would not survive having its cell relocated. Memory absorbs any imbalance between input sides. | runtime |
| S5 | A station's **cell states** are read and written only under that station's mutex; its value **bytes** are protected by ownership instead. | One lock per station, covering every port's states together, so a claim holds exactly one lock and no lock ordering exists to get wrong. A cell in *reserved* or *claimed* belongs to exactly one worker, so the copies into and out of it need no exclusion and happen outside the lock — which is what keeps the expensive half of a delivery off the serialized path. A delivering writer takes no lock at all, because writing touches one cell and one cell is already atomic. Issue 210d. | structure |
...
| T6 | A ring value claimed by one task cannot be claimed by another. | The claim runs under the station's mutex, and a cell moves out of *ready* there. After issue 210c a cell carries its own state rather than being implied by a read index, so "claimed" is a fact written on the cell rather than a position somebody computed &mdash; which is what lets the claimer walk away with it and copy outside the lock. | structure |
...
| # | always true | what it costs | held by |
|---|---|---|---|
| C1 | Every size and offset in the registry is computed by the C compiler, never by the generator. | They are emitted as `sizeof` and `offsetof` expressions. The generator cannot guess wrong because it does not guess. | build |
...
cell rather than computing where the oldest one must be, and a claim
that rolls back releases cells wherever they sit, so gaps open and
...
- [001 — Overview](001-overview.md), the one rule everything else falls
  out of
- [002 — Stations and slots](002-stations-and-slots.md), where the
