...
| S7 | A program is reached only through its marked input and output ports. | Everything else about a program &mdash; a station index, a pointer into its buffers, anything a caller held on to &mdash; is **undefined** to reach across a program boundary, and reaching one across a removal is undefined behaviour rather than a defended case. This is what makes issue 216's wire walk sufficient: a wire lives on an output port and can be found, and a caller outside the map cannot, because nothing on the receiving side remembers who called (issue 213). **It is enforced rather than merely undefined**, which moved this from a discipline to a structure: delivering from outside refuses any port a program did not mark as an argument, and that refusal is the whole of what turns internals that happen to be reachable into a surface (issues 209, 213). The mark is on a **port** now rather than on a station (issues 213a, 209a), which is what made an argument cost nothing: it used to sit on a station, and a station carries one value inward, so a map taking three arguments needed three stations running the identity function. Being an argument is derived where it has to be — a marked port that something inside feeds is not a way in from outside — so an enclosing map wiring into a sub-map needs no bookkeeping and nothing can go stale. The outward mark adds **no** rule of its own: a marked output discards like any other until a caller registers somewhere to put the values, which is the arrow that used to be missing. What is still undefined is a caller that held on to something and reaches past the marked ports anyway; the engine no longer offers it a way to. | structure |
...
| T5 | A task comes into existence by exactly one path. | Everything that starts a station — a delivery arriving, a static being written, the statics bound while a program is built, a captured queue being put back — reaches the same readiness check and the same task build. One path, not several. | structure |
| T6 | **A station that is ready several times over starts that many tasks.** | The one rule says a station runs whenever every port holds a value, and says nothing about how many times in a row. Asking once is enough when values arrive one at a time, because each arrival asks again — and stops being enough when a port fills *all at once*, which is what binding a constant, writing one, or reviving a captured program all do. Thirty values stacked on one port and a constant arriving on the other means thirty tasks, not one. A station whose every port is a constant is the exception and is asked exactly once per change: a constant is never consumed, so such a station is ready forever and draining it would never finish. Issue 712. | structure |
...
| G10 | A static value belongs to exactly one port. | A constant is written on the input line of the port that holds it, so there is no longer any way for two ports to name one value in a file at all: the `statics` section that made that possible was a second spelling of a constant and is gone. Sharing is drawn as a wire instead, which costs a station and gains something visible in the picture. What was a rule about two ports staying independent after the copy is now a property of the notation. | structure |
| G11 | ~~A box cannot write a static.~~ **Retired.** A map with marked input and output ports *is* a box, so a box reaching a map is a box reaching a box, and the prohibition was protecting a distinction that no longer exists. | What it originally bought was G12, and G12 survives without it: a map handle travels as an ordinary value on an ordinary wire, arriving at a marked input port like anything else from outside, so no process-wide pointer returns. A box that builds stations therefore has a side effect far larger than any other box, and that is now deliberate rather than forbidden &mdash; it is what lets a program restructure itself while it runs. | structure |
...
| C6 | ~~A program says where its results come from, or it does not run.~~ **Retired** (issue 209a). | What it bought was that a program's interface is **total** — that "I produce nothing" and "I forgot to say" were different statements. An interface made of numbered ports is total by being read: a map with no result mark produces nothing outward, and that is a complete sentence. What replaced it is a check the old scheme could not make at all: the marked ports are numbered without gaps and without repeats, so two ports both claiming to be argument one is refused, and so is argument two with no argument one. Under the old scheme the order was the order stations happened to sit in the table, and neither mistake was detectable. | load |
...
**Two results of one program are not synchronised with each other**
(issue 209a). They are two stations, on two threads, at two unrelated
moments; nothing pairs them, and nothing ever will, because that is the
same non-guarantee as G-values above wearing a different hat.

The place it will bite is the shape of the collecting API: an embedding
caller registers one array per result, and **two arrays filling side by
side look like columns of a table.** They are not. Entry three of one
and entry three of another did not come from the same input and are not
related in any way.

The rule for anyone building on it: **make the outputs fungible.** If
two values have to stay together, they have to be one value — one
struct out of one station, taken apart by whoever receives it.
