...
5. Confirm the dump still round-trips after a batch rewire, since the
   destination order it relies on is now an array's order rather than a
   list's append order.

## Open questions

- Destinations were appended at the list's tail so wiring order was
  preserved, and the dump's round-trip test leans on that. An array
  built fresh per rewire has to preserve the same order, which is easy
  — but it is an invariant that used to be structural and is now
  something the rebuild has to remember.
