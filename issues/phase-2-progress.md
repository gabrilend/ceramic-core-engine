...
| [215 — ports and slots](215-ports-and-slots.md) | **code and documents done**, one question open | The source called a port a slot and a slot a cell, backwards from what the documents said. Now: a port is a standing interface, a slot is one place one value sits, and the word *cell* is gone from the project. Taken before the rest of the family so the work still to come is written in the words its own blueprints use. What is left is that those blueprints have not been rewritten themselves. |
