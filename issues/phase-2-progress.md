...
| [212 — one way to build a program](212-one-way-to-build-a-program.md) | **in progress** | The capstone. Create, configure, wire — legal at any moment, loading as one caller. The whole-program pass is out of the loader and repeatable, so *still loading* is no longer a state anything can be in; the wire operations and the identical-dumps proof remain. |
