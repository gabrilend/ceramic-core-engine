...
| [217 — a program inside another](217-a-program-inside-another.md) | **built**, two questions open | A program brought inside another is a **template instantiated**: build its stations and wire them the way its description says, translating its own numbers to wherever they landed. Nothing that exists is renumbered. The offset the issue described turned out to be a *table* — a freed place is handed back before the table grows — and the offset is what it degenerates to when nothing has been removed. A parent gets the doors and nothing else. The dump makes names unique on the way out, which is what lets a program holding two copies of one description still be written down. |
