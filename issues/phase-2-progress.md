...
| [203a — a declared depth travels](completed/203a-a-declared-depth-travels.md) | **complete** | Saying how deep one port should be says it for the chain below it, carried by two moments: declaring a depth, and drawing a wire. The second is what makes a file size itself — loading draws every wire — and what makes a station added at minute ten as deep as one added at minute zero. Each kind passes it on the way its own routing works: a plain station gives the whole backlog to every wire, an iterator divides it among its exits, a comparator stops because any of its three could take everything, and the minimum across gating ports is what a station can produce. The fixed point is "already deep enough", which is also what makes a cycle safe. |
| [209a — results go where the caller says](completed/209a-results-go-where-the-caller-says.md) | **complete** | The result mark moved onto a port, and the unbounded holding array went with the warning it used to shout. Nothing is held unless a caller registers an address and a count; a marked output with nowhere registered discards like any other unwired value, so the state the warning described cannot happen. The caller owns the memory, the bound is one atomic add at the reservation rather than the winding down, and there is no slot state machine because a collection slot is written once. The requirement that a program declare a result was retired: an interface made of numbered ports is total by being read. |
| [213a — a door is a port](completed/213a-a-door-is-a-port.md) | **complete** | The entry mark moved off the station and onto a port, and an argument stopped costing a station running the identity function — a mutex, a ring buffer, and per value a task, a dispatch, a call returning its argument, a readiness check and a second delivery, all of it the mark having nowhere smaller to live. The number is stated rather than derived from line position, so reordering a sub-map no longer swaps the parent's arguments; gaps and repeats are refused, neither of which was detectable before. Being an argv slot is derived from what feeds the port, which is what makes composition need no bookkeeping. |
| [216a — removing several stations at once](completed/216a-removing-several-stations-at-once.md) | **complete** | One call takes a set and sweeps the table once, instead of once per member; removing a single station became the set-of-one case. Every member is marked before any wire is cut, and every index is checked before anything is marked, so a bad member refuses the whole set rather than leaving half a program. The back-reference that would avoid the sweep was weighed and refused: delivery only ever asks where a value goes, and paying for the other direction on every wire operation to speed up a rare one is the wrong trade. |
| [212a — one table, and a program is a receipt](completed/212a-one-table-and-a-program-is-a-receipt.md) | **complete** | One station table; everything is wirable to everything. Starting a program beside another is retired — the restriction was real and its reason sound, a wire being an index that only means something inside one table, but what a programmer met was an engine refusing to connect two boxes for a reason living in its memory layout. What it offered is what pruning a receipt now offers: a program is the list of stations one placing created, kept rather than discarded, and ending it is one sweep that cuts every wire naming any member. Two maps in one process is untouched; only the restriction went. Surfaced a double free reachable the moment a program could end by pruning — destroy walked stations before sweeping the scrapyard, and a removed station's shim is not cleared until the sweep runs. |
| [217a — one receipt for a box and a map](completed/217a-one-receipt-for-a-box-and-a-map.md) | **complete** | Placing a box and placing a map both hand back a part number, and the caller cannot tell which it placed. A box's doors are its ports and a map's are its marks — one rule, *the doors are wherever the description put them*, and a description of one station puts them on that station. A number rather than the list because a part travels on a wire when a map builds a map. Left standing: placing a map brings its door marks with it, so an unwired one becomes a way out of the program that placed it, which is right and is also a door nobody asked for. |
| [217 — a program inside another](completed/217-a-program-inside-another.md) | **complete** | **Adding a box and adding a map are one operation**: a map is a list of boxes and the wiring between them, a box is a list of one. What comes back is a *part* — where values go in and where they come out — and for a single box those are the same station, because a box's own ports are its own doors. A part is never taken apart; everything that consumes one takes it whole, so no accessor boxes exist. The offset the issue described turned out to be a translation *table*, because a freed place is handed back before the table grows. The dump makes names unique on the way out, which is what lets a program holding two copies of one description still be written down. |
