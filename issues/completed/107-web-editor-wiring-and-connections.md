...
`assets/js/006-wires.js` handles bezier wire rendering (S-curve with horizontal control point offsets), port hit detection using sampled points along the curve, and connection create/delete with fetch-before-PUT to minimize overwrite races. Creating or deleting a wire updates both endpoint box files. Wire hit testing samples 20 points along the curve and checks distance against a 6px threshold.
