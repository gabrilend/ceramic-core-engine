...
no slot moves, and the hazard stops existing — publish before copying
...
read contiguously from slot zero, and the indices are corrected. Only
the storage the port points at is reallocated — never the port, never
the station — which the station-table test proves by flooding one
port through seven doublings and finding every address unchanged and
...
report is a read rather than a retrofit) live on the port. The
wrapped-before-growth case is exactly what the ports test seeds.
...
reallocates the *storage the port points at*, not the station. Every
...
variable parts hang off pointers — see issue 201. Had the slots been
...
**Growth happens under the mutex that already guards the port.** Every
...
place — a count of how many times each port has grown — so that
reporting it later is a read rather than a retrofit.

## Suggested implementation steps

1. Add a growth count to the port struct.
2. In write, replace the loud failure with: reallocate to twice
   ...
   the growth count.
3. A test that fills a port of small capacity, grows it several times,
...
4. A test that grows a port while other threads are writing to
   neighbouring ports on the same station, confirming the station's
   address and every other port's contents are untouched.
