...
**Done.** A description is compiled into the calls it describes,
whether at build time or while a program runs, and the engine's own
way of reading one is deleted.
...
text makes ([217](217-a-program-inside-another.md)).
...
One table, of 1.6 kilobytes, mapping a short box name to a full
address. It holds no code alive and answers a question nothing else
answers. Step 11 below says why it stays and what it would cost to
remove.
...
[217](217-a-program-inside-another.md) makes from the other
direction.
...
8. **Done. One question, asked over everything the program is made
   of.** The sources the build compiled in ([311c](311c-source-rides-in-the-binary.md))
...
11. **Dropped, like step 5, and for the same kind of reason: the work
    below it moved the ground.**

    The step existed to serve the parent's claim that a running program
    holds no name and no lookup. **That claim was spent before this
    step was reached.** Extendability requires publishing the functions
    that build stations, so that a description compiled next year can
    bind to a box compiled today — and a program that publishes
    twenty-two symbol names does hold names. The dynamic symbol table
    *is* a lookup, maintained by the operating system.

    **So the question became what the table still does that the
    published names do not**, and the answer is: it maps a *short name*
    to an *address*. It is the namespace [311a](311a-boxes-addressed-by-file.md)
    said existed but that nobody had written down. The dynamic linker
    cannot do that job — it resolves an exact symbol or nothing.

    What it costs is now almost nothing, and measurably so: 528 bytes
    of rows and 1,134 bytes of strings, and **it holds no code alive**,
    because the station-builders it points at are roots already.

    What deleting it would cost is real: either every caller writing C
    spells out a full address — 117 places across 17 files — or the
    rule for turning an address into a symbol gets a second
    implementation inside the engine, which is a rule that must never
    drift between two copies.

    A table that costs 1.6 kilobytes, roots nothing, and answers a
    question nothing else answers is not the thing this family was
    written to remove. What it was written to remove — a record of what
    every box *is*, read once and never again, holding every box in the
    binary alive — is gone.
...
13. **Done already, and by the stronger comparison**: a program built
    ...
    the read form
    ([212](212-one-way-to-build-a-program.md)), so this
