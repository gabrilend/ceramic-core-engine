...
classify-demo map (2026-05-07): 5-box pipeline exercising lua, bash, and branch
routing with data file access. Entry box `read-input` has no wired inputs —
reads name from data/input.json via SORAMECH_MAP_DIR. Bash `stamp` function
outputs a JSON array `["..."]` directly (required format for bash driver output).
Router predicate `contains "SoraMech"` lets both paths be exercised by changing
data/input.json.

Both paths verified:
- name="SoraMech" → greeting port fires → write-result writes to data/output.json
- name="stranger" → else port fires → write-error writes to data/error.json

`demo.sh` in project root runs the phase 1 demo.
Full documentation in issues/completed/demos/phase-1-classify-demo/README.md.

Issue 111 created for bash output format confusion in driver-test map.
