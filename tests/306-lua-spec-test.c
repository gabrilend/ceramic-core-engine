...
static int test_invoke_greet_both_args(void)
{
    ...
    int rc = lua->invoke(h, "tests/maps/hello/src/hello.lua", "greet",
                         args, sizes, 2,
                         buf, sizeof buf, &n);
    ASSERT(rc == 0);
...
static int test_invoke_missing_function(void)
{
    ...
    char buf[64];
    int  n = 0;
    int rc = lua->invoke(h, "tests/maps/hello/src/hello.lua", "no_such_fn",
...
static int test_invoke_missing_file(void)
{
    ...
    char buf[64];
    int  n = 0;
    int rc = lua->invoke(h, "tests/maps/nope/missing.lua", "greet",
                         NULL, NULL, 0, buf, sizeof buf, &n);
    ASSERT(rc != 0);
