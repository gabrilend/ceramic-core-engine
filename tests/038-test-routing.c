...
static void less_sink__call(cera_task_t *t)   { (void)t; hits_less++; }
static void equal_sink__call(cera_task_t *t)  { (void)t; hits_equal++; }
static void greater_sink__call(cera_task_t *t){ (void)t; hits_greater++; }
/* }}} */
...
static void wire_three_sinks(cera_map_t *m, int station, int base, int value_size)
{
    int one[1] = { value_size };
    cera_map_place(m, base + 0, less_sink__call, CERA_STATION_PLAIN, 1, one, 0);
    cera_map_place(m, base + 1, equal_sink__call, CERA_STATION_PLAIN, 1, one, 0);
    cera_map_place(m, base + 2, greater_sink__call, CERA_STATION_PLAIN, 1, one, 0);
    cera_map_connect(m, station, 0, base + 0, 0);
    cera_map_connect(m, station, 1, base + 1, 0);
    cera_map_connect(m, station, 2, base + 2, 0);
}
/* }}} */
...
static void test_three_outcomes(void)
{
    ...
    cera_map_place_box(m, 0, "add", CERA_STATION_COMPARATOR);
    wire_three_sinks(m, 0, 1, sizeof(int));

    cera_map_in_port_static_text(m, 0, 2, "100");   /* the threshold port, last */
    ...
    for (int i = 0; i < 5; i++) {
        ...
        cera_map_deliver_value(m, 0, 1, &below);
        cera_map_deliver_value(m, 0, 0, &a);
        cera_map_deliver_value(m, 0, 1, &exact);
        cera_map_deliver_value(m, 0, 0, &a);
        cera_map_deliver_value(m, 0, 1, &above);
    }
...
static void test_buffered_threshold_blocks(void)
{
     ...
     * empty when released... */
    int threshold = 10;
    cera_map_deliver_value(m, 0, 2, &threshold);
    cera_pool_release(m->pool);
    cera_pool_join(m->pool);
...
static void test_unwired_port_discards(void)
{
    ...
    cera_map_place(m, 1, greater_sink__call, CERA_STATION_PLAIN, 1, one, 0);
    cera_map_connect(m, 0, 2, 1, 0);

    cera_map_in_port_static_text(m, 0, 2, "100");
    ...
    cera_map_deliver_value(m, 0, 0, &big_a);
    cera_map_deliver_value(m, 0, 1, &big_b);
    cera_pool_release(m->pool);
    cera_pool_join(m->pool);
...
static void test_float_semantics(void)
{
    ...
    cera_map_t *m = cera_map_create(4);
    cera_map_place_box(m, 0, "mix", CERA_STATION_COMPARATOR);
    wire_three_sinks(m, 0, 1, sizeof(double));

    cera_map_in_port_static_text(m, 0, 2, "0.5");

    cera_map_start(m, 2);
    reset_hits();
    int count = 1;
    double factor = -2.0;          /* mix -> -2.0 */
    cera_map_deliver_value(m, 0, 0, &count);
    cera_map_deliver_value(m, 0, 1, &factor);
    cera_pool_release(m->pool);
    cera_pool_join(m->pool);
...
static void test_struct_author_order(void)
{
     ...
     * magnitude says less. */
    cera_map_t *m = cera_map_create(4);
    cera_map_place_box(m, 0, "make_vec3", CERA_STATION_COMPARATOR);
    wire_three_sinks(m, 0, 1, sizeof(vec3));

    cera_map_in_port_static_text(m, 0, 3, "{ 1, 8, 8 }");   /* magnitude^2 = 129 */

    cera_map_start(m, 2);
    reset_hits();
    float x = 9, y = 0, z = 0;                  /* magnitude^2 = 81 */
    cera_map_deliver_value(m, 0, 0, &x);
    cera_map_deliver_value(m, 0, 1, &y);
    cera_map_deliver_value(m, 0, 2, &z);
    cera_pool_release(m->pool);
    cera_pool_join(m->pool);
...
static void port0_sink__call(cera_task_t *t) { (void)t; port_counts[0]++; }
static void port1_sink__call(cera_task_t *t) { (void)t; port_counts[1]++; }
static void port2_sink__call(cera_task_t *t) { (void)t; port_counts[2]++; }

static cera_map_t *crowd_map;
...
static void *crowd_feeder(void *arg)
{
    ...
    for (int i = 0; i < 300; i++) {
        int v = base + i;
        cera_map_deliver_value(crowd_map, 0, 0, &v);
    }
    return NULL;
}
...
static void test_iterator_spreads(void)
{
    ...
    crowd_map = cera_map_create(4);
    cera_map_place_box(crowd_map, 0, "double_it", CERA_STATION_ITERATOR);
    int one[1] = { sizeof(int) };
    cera_map_place(crowd_map, 1, port0_sink__call, CERA_STATION_PLAIN, 1, one, 0);
    cera_map_place(crowd_map, 2, port1_sink__call, CERA_STATION_PLAIN, 1, one, 0);
    cera_map_place(crowd_map, 3, port2_sink__call, CERA_STATION_PLAIN, 1, one, 0);
    cera_map_connect(crowd_map, 0, 0, 1, 0);
    cera_map_connect(crowd_map, 0, 1, 2, 0);
    cera_map_connect(crowd_map, 0, 2, 3, 0);

    cera_map_start(crowd_map, 0);
    port_counts[0] = port_counts[1] = port_counts[2] = 0;

    cera_pool_submitter_register(crowd_map->pool);
    cera_pool_release(crowd_map->pool);
    ...
    cera_pool_submitter_unregister(crowd_map->pool);
    cera_pool_join(crowd_map->pool);
