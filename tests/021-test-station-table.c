...
static void unused__call(cera_task_t *t) { (void)t; }

int main(void)
{
    enum { STATIONS = 3 };
    cera_map_t *m = cera_map_create(STATIONS);

    /* Two int inputs each; nothing wired, nothing started. */
    int sizes[2] = { sizeof(int), sizeof(int) };
    for (int i = 0; i < STATIONS; i++)
        cera_map_place(m, i, unused__call, CERA_STATION_PLAIN, 2, sizes, sizeof(int));

    /* Write down where everything lives. */
    cera_station_t *before[STATIONS];
    for (int i = 0; i < STATIONS; i++)
        before[i] = cera_map_station(m, i);

    /* Leave a fingerprint in station 2's first port. */
    int fingerprint = 777;
    cera_map_deliver_value(m, 2, 0, &fingerprint);
